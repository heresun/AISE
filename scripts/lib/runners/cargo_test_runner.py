...
def invoke(ctx) -> tuple[int, dict]:
    ...
    ok_rt, rt_info = er.resolve_runtime_bin("cargo-test-junit", ctx.project_root)
    if not ok_rt:
    ...
    return run(
        ...
        cargo2junit_bin=ctx.preflight_info["found"],
        extra_args=ctx.extra.get("cargo_extra_arg", []),
    )
